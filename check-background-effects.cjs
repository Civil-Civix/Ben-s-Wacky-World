...
assert.match(await p.locator('.home-title').evaluate(e=>getComputedStyle(e,'::before').backgroundImage),/32, 195, 146/);
...
await p.locator('.nav-game').click();await p.locator('.game-card').first().waitFor();assert.equal(await p.locator('.game-card').count(),401);
...
assert.deepEqual(errors,[]);console.log('PASS: four effects move, pause, persist, follow accent, and stop during games; title tint and spotlight; 401 games; no page errors.');
